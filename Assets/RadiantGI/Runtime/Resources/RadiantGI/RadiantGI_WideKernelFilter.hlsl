...
// blur sample stride uniform: lower value = denser sampling, better quality
int _BlurSampleStep;
...
#if defined(_BLUR_QUALITY_HIGHEST)
    // Highest path: fully unrolled loop with step 1 (63 inlined iterations)
    UNITY_UNROLL
    for (int k = 1; k < 64; k++) {
#else
    // High / Faster path: dynamic loop with uniform step (2 or 4).
    // Rotate the starting index per-frame so the temporal filter converges to
    // coverage of all 63 offsets, removing the visible stride pattern.
    int kStart = 1 + ((int)FRAME_NUMBER & (_BlurSampleStep - 1));
    UNITY_LOOP
    for (int k = kStart; k < 64; k += _BlurSampleStep) {
#endif
