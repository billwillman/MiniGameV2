...
public class BaseResLoader: CachedMonoBehaviour
{
	...
	public void ClearScriptObject(ICustomLoaderEvent target, string resName, string resTag) {
		if (target == null || target.CustomLoaderBehaviour == null)
			return;
		SetResource(target.CustomLoaderBehaviour.GetInstanceID(), null, typeof(ScriptableObject), resName, resTag);
	}
	...
	protected string m_CurrentMainSceneAB = string.Empty;

	public string CurrentMainSceneAB {
		get {
			return m_CurrentMainSceneAB;
		}
	}

	public void ClearMainSceneAB() {
		if (!string.IsNullOrEmpty(m_CurrentMainSceneAB)) {
			ClearSceneAB(m_CurrentMainSceneAB);
			m_CurrentMainSceneAB = string.Empty;
		}
	}
