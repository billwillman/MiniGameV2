...
namespace HybridCLR.Editor
{
    public static class SettingsUtil
    {
        ...
        /// <summary>
        /// HybridCLR package root (UPM Packages/ or Assets/hybridclr).
        /// </summary>
        public static string PackageRoot
        {
            get
            {
                string projectDir = ProjectDir.Replace('\\', '/');
                string embedded = $"{projectDir}/Packages/{PackageName}";
                if (File.Exists($"{embedded}/package.json"))
                    return embedded;
                string inAssets = $"{projectDir}/Assets/hybridclr";
                if (File.Exists($"{inAssets}/package.json"))
                    return inAssets;
                return embedded;
            }
        }
        ...
        /// <summary>
        /// Path relative to project root, used as `{ProjectDir}/{HybridCLRDataPathInPackage}/...`.
        /// </summary>
        public static string HybridCLRDataPathInPackage
        {
            get
            {
                string projectDir = ProjectDir.Replace('\\', '/');
                string root = PackageRoot.Replace('\\', '/');
                if (root.StartsWith(projectDir, StringComparison.OrdinalIgnoreCase))
                {
                    string rel = root.Substring(projectDir.Length).TrimStart('/');
                    return $"{rel}/Data~";
                }
                return $"Packages/{PackageName}/Data~";
            }
        }
